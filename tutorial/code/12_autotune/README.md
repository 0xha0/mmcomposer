...
| `NUM_WARPS` (epilogue warps)     | 4, 8, 16         | ch10 | held at 8 |
| `LD_X` (`tcgen05.ld` packing)    | 8, 16, 32, 64    | ch10 | held at 8 |

`NUM_WARPS = 4` was pruned in ch10 (never wins).  `NUM_WARPS = 16` we
tried adding to the autotune sweep and the autotuner did pick it at
4 of 11 shapes — but a controlled head-to-head probe at fixed
`(NS, GSM)` showed NW=16 is within ±2 % of NW=8 at every shape, same
signature as `LD_X` (irrelevant tunable being chased by tournament
noise).  Dropped.  `LD_X` turned out to be within ~1 % noise across
all values at every shape (see [`probe_ldx.py`](probe_ldx.py)), also
dropped.  That leaves the **`NS × GROUP_SIZE_M`** cross to tune.
...
|---|---|---|---|---|
| 2048³  | (5, 4)  |  **801** |  874 |  92 % |
| 3072³  | (7, 4)  | **1205** | 1379 |  87 % |
| 4096³  | (6, 1)  | **1208** | 1328 |  91 % |
| 5120³  | (7, 8)  | **1231** | 1386 |  89 % |
| 6144³  | (7, 8)  | **1294** | 1403 |  92 % |
| 7168³  | (6, 8)  | **1306** | 1377 |  95 % |
| 8192³  | (6, 8)  | **1314** | 1347 |  98 % |
| 9216³  | (6, 8)  | **1307** | 1358 |  96 % |
| 10240³ | (7, 8)  | **1301** | 1309 |  99 % |
| 11264³ | (5, 8)  | **1303** | 1312 |  99 % |
| 12288³ | (6, 8)  | **1327** | 1409 |  94 % |

Three things to read off:

- **The picked config varies by shape.** No single `(NS, GSM)` is
  optimal everywhere — that's the whole point of autotuning.  Small
  shapes prefer `GSM = 1` or `GSM = 4` (the chunked walk has little
  L2 reuse to give it); larger shapes consolidate on `GSM = 8`.
