...
//
// Identical to ch09 (CTA swizzle + 2-CTA cluster + multi-stage +
// warp-spec) inside the K-loop.  Two new template parameters affect
// only the epilogue:
//
//   * NUM_WARPS (4 or 8) — total warps per CTA.  Main loop is
//     unchanged (warp 0 = TMA, warp 1 = MMA, others idle); the
//     extra warps participate only in the epilogue.  With 8 warps,
//     phase 1 (TMEM → SMEM) splits both M-rows and N-cols across
//     warps; phase 2 (SMEM → GMEM) halves the per-thread store
//     count.
//
//   * LD_X (8, 16, 32, 64) — tcgen05.ld packing factor.  Wider loads
//     fetch more cols of TMEM per call (32 lanes × LD_X cols × 4 B
//     per call), reducing the number of ld + wait pairs at the cost
//     of more registers per lane.
//
// Both NS and GROUP_SIZE_M are fixed at ch09's best (NS=5, GSM=8 at
// 8192³).  Launchers are emitted for the full NUM_WARPS × LD_X
// cross.
...
// Launchers — full cross of NUM_WARPS × LD_X at fixed NS=5, GSM=8.
#define MAKE_LAUNCHER(NW_, LDX_)                                                          \
extern "C" __global__ __cluster_dims__(CTA_GROUP, 1, 1)                                   \
__launch_bounds__(NW_ * WARP_SIZE, 1)                                                     \
void matmul_epi_nw##NW_##_ldx##LDX_(                                                      \
