...
__device__ __forceinline__ void matmul_cluster_impl(
    ...
    const uint32_t taddr_row_base =
        taddr + ((uint32_t)(cta_rank * BM + warp_id * 32) << 16);
...
#define MAKE_LAUNCHER(NS_, GSM_)                                                       \
extern "C" __global__ __cluster_dims__(CTA_GROUP, 1, 1) __launch_bounds__(THREADS, 1)  \
void matmul_swizzle_ns##NS_##_gsm##GSM_(                                               \
    const __grid_constant__ CUtensorMap A_tmap,                                        \
    const __grid_constant__ CUtensorMap B_tmap,                                        \
    __nv_bfloat16* C_ptr, int M, int N, int K)                                         \
{                                                                                      \
    matmul_cluster_impl<NS_, GSM_>(&A_tmap, &B_tmap, C_ptr, M, N, K);                  \
}
