...
\begin{frame}{Epilogue Concern 2: Filling the $128\times64$ SMEM Buffer}
  ...
  Shown is one phase (lanes 0--7); a 128-bit store issues as 4 identical phases,
  so rows sharing a group never write together.}
\end{frame}

\begin{frame}{Epilogue Concern 2: Why the Period Is 8 --- an $8\times$ Win}
  A warp writes 32 rows, but an \code{int4} (16\,B) SMEM store moves only
  128\,B/cycle, so it issues as \textbf{4 phases of 8 lanes}:
  \[
    \underbrace{16\,\text{B}}_{\mathtt{int4}} \times
    \underbrace{8\,\text{lanes}}_{\text{per cycle}}
    = \underbrace{128\,\text{B} = 32\,\text{banks}}_{\text{one bank cycle}}
    = \underbrace{\mathtt{SWIZZLE\_128B}}_{\text{TMA atom}}
  \]
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{tabular}{ccc}
      ...
      \bottomrule
    \end{tabular}\\[0.15cm]
    {\scriptsize each phase: \code{lane\,\&\,7} $=0\ldots7$, all distinct}
    ...
    \begin{tabular}{lcc}
      \toprule
       & per phase & 4 phases \\
      \midrule
      naive & 8-way $\to$ 8\,cyc & \textbf{32 cyc} \\
      swizzle & conflict-free $\to$ 1\,cyc & \textbf{4 cyc} \\
      \bottomrule
    \end{tabular}\\[0.15cm]
    {\scriptsize the 4 phases serialize either way --- the swizzle just makes
    each one conflict-free}
  \end{columns}

  \vspace{0.25cm}
  \centering
  \small Period 8 is chosen to match the 8-lane phase $\Rightarrow$ $8\times$
  faster SMEM packing, with the TMA$\to$HBM store coalesced either way.
\end{frame}
