...
def validate_config(bm, bn, bk, ns, gsm, nw, *, cluster: bool) -> list[str]:
...
def knob_kwargs(bm, bn, bk, ns, gsm, nw) -> dict:
    """Map UI knobs to the constant names used in the source files."""
    return {"BM": bm, "BN": bn, "BK": bk, "NS": ns, "GROUP_SIZE_M": gsm, "NUM_WARPS": nw}
...
def render_host(tier: dict, bm, bn, bk, ns, gsm, nw) -> str:
    ...
    launcher = substitute_launcher_constants(launcher, **knob_kwargs(bm, bn, bk, ns, gsm, nw))
    header = (
...
def _compat_index() -> dict:
    """(tier_dir, bm, bn, bk, ns, gsm, nw) -> entry dict."""
    idx = {}
    for e in load_compat().get("entries", []):
        idx[(e["tier"], e["bm"], e["bn"], e["bk"], e["ns"], e["gsm"], e["nw"])] = e
    return idx


def compat_status(tier_dir, bm, bn, bk, ns, gsm, nw):
