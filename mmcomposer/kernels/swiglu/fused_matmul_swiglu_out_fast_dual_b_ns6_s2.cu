...
constexpr int GROUP_SIZE_M = 1;       // CTA-swizzle chunk (1 = no swizzle)
...
__device__ __forceinline__ void matmul_cluster_impl(
    ...
    //
    // Grid is ceil(M / (CTA_GROUP*BM)) * (N / BN) flat CTA ids.  Each
...
{
    matmul_cluster_impl(&A_tmap, &B_left_tmap, &B_gate_tmap, &C_tmap, &D_tmap, C_ptr, M, N, K);
}
