...
/*
 * h2_s8_smem_wb_swz: h2_s8_smem_wb + Triton-style GROUP_M CTA-id remap.
 *
 * Instead of the natural row-major (m,n) tile mapping, we re-linearise the
 * CTA id so each block of GROUP_M*num_pid_n CTAs covers a GROUP_M ×
 * num_pid_n band of output tiles, iterating M-first within the band. A wave
 * of 132 CTAs then hits a smaller (M_tiles × N_tiles) bounding box → higher
 * L2 hit rate, especially at larger N where the natural row-major scan
 * blows past L2 between waves.
 *
 * Autotunes over GROUP_M ∈ {1, 2, 4, 8} (1 = no swizzle, matches s8_smem_wb).
 */
                ...
                for (int ktile = 0; ktile < num_k_tiles; ktile++) {
                    ...
                    for (int nst = 0; nst < N_SUBTILES; nst++) {
                        tma_2d_load(bbase + nst * BK * STORE_N * BF16_BYTES,
                                    B_tmap, block_col + nst * STORE_N, ktile * BK, ready_mb);
                    }
...
#define WAIT_MMA(n_) \
    ...
    for (int tile_id = (int)blockIdx.x; tile_id < total_tiles; tile_id += (int)gridDim.x) {
        ...
        constexpr int CHUNKS = BN / STORE_N;
        ...
        for (int chunk = 0; chunk < CHUNKS; chunk++) {
            ...
            if (tid == 0) {
                ...
                tma_2d_store(C_tmap, src, block_col + chunk * STORE_N, block_row);
                tma_commit_group();
            }
            math_barrier();
        }
...
void matmul_hopper_ws_tma_load_store_bm128_bn256_bk64_wg2_ns4_gm##GM_(          \
    const __grid_constant__ CUtensorMap A_tmap,                                  \
    const __grid_constant__ CUtensorMap B_tmap,                                  \
    const __grid_constant__ CUtensorMap C_tmap,                                  \
    int M, int K, int N)                                                         \
{                                                                                \
    hopper_ws_tma_load_store_impl<128, 256, 64, 2, 4, GM_>(                      \
        &A_tmap, &B_tmap, &C_tmap, M, K, N);                                     \
}
